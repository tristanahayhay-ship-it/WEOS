...
import {
  buildApproximateBoundaryRings,
} from '../utils/adminBoundaryApproximator'
...
/**
 * Build an AdministrativeDivision entry with seeded economic metrics and an
 * approximate boundary ring derived from the centroid + radius estimate.
 *
 ...
 * @param radiusDeg    Radius of the approximate boundary polygon in degrees
 *                     (latitude units).  Computed from country area / n
 *                     if omitted.  Pass explicitly for best visual results.
 */
function buildDivision(
  ...
  const r = radiusDeg ?? 1.5   // conservative fallback: 1.5° ≈ 165 km
  return {
    ...
    boundaryRings: buildApproximateBoundaryRings(center, r),
  }
}

// Country area (km²) → per-division radius (degrees), formula:
//   r_km  = sqrt( area / n_divisions / π )
//   r_deg ≈ r_km / 111
//
// JP 377 972 km² / 10  → ~1.0°   US 9.83 M km² / 10 → ~5.0°
// CN 9.60 M km² / 10  → ~5.0°   DE 357 114 km² / 8  → ~1.1°
// IN 3.29 M km² / 10  → ~2.9°   BR 8.52 M km² / 8   → ~5.2°
// RU 17.1 M km² /  5  → ~9.4°   AU 7.69 M km² / 6   → ~5.7°
// CA 9.98 M km² /  5  → ~7.2°
...
const JP_ADMIN: CountryAdminData = {
  countryIsoCode: 'JP',
  divisions: [
    buildDivision('JP', 'prefecture', 'Tokyo',    [139.69,  35.69], 0, 1.0),
    buildDivision('JP', 'prefecture', 'Osaka',    [135.50,  34.69], 1, 1.0),
    buildDivision('JP', 'prefecture', 'Kanagawa', [139.64,  35.45], 2, 1.0),
    buildDivision('JP', 'prefecture', 'Aichi',    [137.01,  35.18], 3, 1.0),
    buildDivision('JP', 'prefecture', 'Saitama',  [139.65,  35.86], 4, 1.0),
    buildDivision('JP', 'prefecture', 'Chiba',    [140.12,  35.61], 5, 1.0),
    buildDivision('JP', 'prefecture', 'Hyogo',    [134.69,  34.69], 6, 1.0),
    buildDivision('JP', 'prefecture', 'Hokkaido', [142.81,  43.06], 7, 1.0),
    buildDivision('JP', 'prefecture', 'Fukuoka',  [130.42,  33.60], 8, 1.0),
    buildDivision('JP', 'prefecture', 'Shizuoka', [138.39,  34.98], 9, 1.0),
  ],
  intraFlows: [
...
const US_ADMIN: CountryAdminData = {
  countryIsoCode: 'US',
  divisions: [
    buildDivision('US', 'state', 'California',   [-119.42,  36.78], 0, 5.0),
    buildDivision('US', 'state', 'Texas',         [-99.34,  31.47], 1, 5.0),
    buildDivision('US', 'state', 'New York',      [-74.95,  43.30], 2, 5.0),
    buildDivision('US', 'state', 'Florida',       [-81.52,  27.99], 3, 5.0),
    buildDivision('US', 'state', 'Illinois',      [-89.20,  40.35], 4, 5.0),
    buildDivision('US', 'state', 'Pennsylvania',  [-77.19,  41.20], 5, 5.0),
    buildDivision('US', 'state', 'Ohio',          [-82.79,  40.42], 6, 5.0),
    buildDivision('US', 'state', 'Georgia',       [-83.64,  32.16], 7, 5.0),
    buildDivision('US', 'state', 'Washington',   [-120.74,  47.75], 8, 5.0),
    buildDivision('US', 'state', 'Massachusetts', [-71.53,  42.23], 9, 5.0),
  ],
  intraFlows: [
...
const CN_ADMIN: CountryAdminData = {
  countryIsoCode: 'CN',
  divisions: [
    buildDivision('CN', 'province', 'Guangdong',  [113.27,  23.13], 0, 5.0),
    buildDivision('CN', 'province', 'Jiangsu',    [119.46,  32.97], 1, 5.0),
    buildDivision('CN', 'province', 'Shandong',   [117.00,  36.66], 2, 5.0),
    buildDivision('CN', 'province', 'Zhejiang',   [120.15,  29.14], 3, 5.0),
    buildDivision('CN', 'province', 'Henan',      [113.75,  34.77], 4, 5.0),
    buildDivision('CN', 'province', 'Sichuan',    [103.00,  30.65], 5, 5.0),
    buildDivision('CN', 'province', 'Hubei',      [112.30,  31.20], 6, 5.0),
    buildDivision('CN', 'province', 'Fujian',     [117.98,  26.10], 7, 5.0),
    buildDivision('CN', 'province', 'Hunan',      [112.98,  27.51], 8, 5.0),
    buildDivision('CN', 'province', 'Shaanxi',    [108.95,  35.17], 9, 5.0),
  ],
  intraFlows: [
...
const DE_ADMIN: CountryAdminData = {
  countryIsoCode: 'DE',
  divisions: [
    buildDivision('DE', 'state', 'North Rhine-Westphalia', [7.53, 51.53], 0, 1.1),
    buildDivision('DE', 'state', 'Bavaria',                [12.00, 48.79], 1, 1.1),
    buildDivision('DE', 'state', 'Baden-Württemberg',      [9.18, 48.78], 2, 1.1),
    buildDivision('DE', 'state', 'Hesse',                  [9.17, 50.65], 3, 1.1),
    buildDivision('DE', 'state', 'Lower Saxony',           [9.75, 52.64], 4, 1.1),
    buildDivision('DE', 'state', 'Berlin',                 [13.40, 52.52], 5, 1.1),
    buildDivision('DE', 'state', 'Saxony',                 [13.40, 51.10], 6, 1.1),
    buildDivision('DE', 'state', 'Hamburg',                [10.00, 53.55], 7, 1.1),
  ],
  intraFlows: [
...
const IN_ADMIN: CountryAdminData = {
  countryIsoCode: 'IN',
  divisions: [
    buildDivision('IN', 'state', 'Maharashtra',      [75.71,  19.75], 0, 2.9),
    buildDivision('IN', 'state', 'Tamil Nadu',       [78.66,  11.13], 1, 2.9),
    buildDivision('IN', 'state', 'Karnataka',        [75.72,  15.32], 2, 2.9),
    buildDivision('IN', 'state', 'Gujarat',          [71.19,  22.26], 3, 2.9),
    buildDivision('IN', 'state', 'Uttar Pradesh',    [80.91,  26.85], 4, 2.9),
    buildDivision('IN', 'state', 'Rajasthan',        [74.22,  27.02], 5, 2.9),
    buildDivision('IN', 'state', 'West Bengal',      [87.85,  22.99], 6, 2.9),
    buildDivision('IN', 'state', 'Telangana',        [79.02,  17.91], 7, 2.9),
    buildDivision('IN', 'state', 'Andhra Pradesh',   [79.74,  15.91], 8, 2.9),
    buildDivision('IN', 'state', 'Madhya Pradesh',   [78.66,  23.47], 9, 2.9),
  ],
  intraFlows: [
...
const BR_ADMIN: CountryAdminData = {
  countryIsoCode: 'BR',
  divisions: [
    buildDivision('BR', 'state', 'São Paulo',       [-48.55, -22.19], 0, 5.2),
    buildDivision('BR', 'state', 'Rio de Janeiro',  [-43.24, -22.91], 1, 5.2),
    buildDivision('BR', 'state', 'Minas Gerais',    [-44.38, -18.51], 2, 5.2),
    buildDivision('BR', 'state', 'Rio Grande do Sul',[-53.08, -30.03], 3, 5.2),
    buildDivision('BR', 'state', 'Paraná',          [-51.61, -25.25], 4, 5.2),
    buildDivision('BR', 'state', 'Bahia',           [-41.70, -12.97], 5, 5.2),
    buildDivision('BR', 'state', 'Pernambuco',      [-37.82,  -8.81], 6, 5.2),
    buildDivision('BR', 'state', 'Goiás',           [-49.32, -15.83], 7, 5.2),
  ],
  intraFlows: [
...
const RU_ADMIN: CountryAdminData = {
  countryIsoCode: 'RU',
  divisions: [
    buildDivision('RU', 'region', 'Central',        [ 37.62,  55.75], 0, 9.4),
    buildDivision('RU', 'region', 'Northwestern',   [ 30.32,  59.94], 1, 9.4),
    buildDivision('RU', 'region', 'Urals',          [ 60.60,  56.84], 2, 9.4),
    buildDivision('RU', 'region', 'Siberia',        [ 82.92,  54.99], 3, 9.4),
    buildDivision('RU', 'region', 'Far East',       [131.87,  43.12], 4, 9.4),
  ],
  intraFlows: [
...
const AU_ADMIN: CountryAdminData = {
  countryIsoCode: 'AU',
  divisions: [
    buildDivision('AU', 'state', 'New South Wales',  [146.92, -32.16], 0, 5.7),
    buildDivision('AU', 'state', 'Victoria',         [144.78, -36.85], 1, 5.7),
    buildDivision('AU', 'state', 'Queensland',       [144.08, -22.58], 2, 5.7),
    buildDivision('AU', 'state', 'Western Australia',[121.63, -25.33], 3, 5.7),
    buildDivision('AU', 'state', 'South Australia',  [135.76, -30.00], 4, 5.7),
    buildDivision('AU', 'state', 'Tasmania',         [146.82, -42.00], 5, 5.7),
  ],
  intraFlows: [
...
const CA_ADMIN: CountryAdminData = {
  countryIsoCode: 'CA',
  divisions: [
    buildDivision('CA', 'province', 'Ontario',          [-85.32,  49.25], 0, 7.2),
    buildDivision('CA', 'province', 'Quebec',           [-72.55,  52.94], 1, 7.2),
    buildDivision('CA', 'province', 'British Columbia', [-124.64,  53.73], 2, 7.2),
    buildDivision('CA', 'province', 'Alberta',          [-113.49,  53.93], 3, 7.2),
    buildDivision('CA', 'province', 'Manitoba',         [ -98.81,  56.41], 4, 7.2),
  ],
  intraFlows: [
