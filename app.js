...
const SieuBanDoKinhTe = Globe()(container)
    .globeImageUrl('//://unpkg.com') // Nền trái đất ban đêm
    .bumpImageUrl('//://unpkg.com')
    .backgroundImageUrl('//://unpkg.com') // Nền vũ trụ sao mờ
    .showAtmosphere(true)
...
function capNhatDoHoaMachMau() {
    SieuBanDoKinhTe
        ...
        // KIẾN TRÚC SINH HỌC 1: Đầu tư -> Máu Xanh, Tháo chạy bảo thủ -> Máu Đỏ
        .arcColor(d => d.loaiDongTien === 'DAU_TU' ? '#00ff66' : '#ff2233')
        
        // KIẾN TRÚC SINH HỌC 2: Tiền bơm/tháo càng mạnh (Volume lớn) -> Mạch máu càng PHÌNH TO
        .arcStroke(d => {
            const doDay = d.volume / 1000000000; // Tỷ lệ thuận theo đơn vị Tỷ USD
            return Math.max(0.4, Math.min(doDay, 4.5)); // Giới hạn độ dày tối đa để tránh loãng hình
        })
        
        // KIẾN TRÚC SINH HỌC 3: Xung chuyển động của các hạt giọt máu trên đường dây
        .arcDashLength(0.5)
        .arcDashGap(0.3)
...
// Code thật - Tự động biến động dòng số và quy mô mạch máu như một cơ thể sống
setInterval(() => {
    ...
    // Re-render (vẽ lại các bó cơ mạch máu) liên tục mà không giật màn hình
    capNhatDoHoaMachMau();
